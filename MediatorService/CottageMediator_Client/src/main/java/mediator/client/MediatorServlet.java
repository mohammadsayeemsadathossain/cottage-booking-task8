...
public class MediatorServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("searchCottageWithMapping".equals(reqType)) {
				...
				Type type = new TypeToken<Map<String, AlignmentCandidate>>() {
				}.getType();
			...
			RequestTemplate template = prepareRequestFromRdg(rdgTurtle, bookerName, numberOfPeople, numberOfBedrooms,
					maxDistanceFromLake, cityName, maxCityDistance, numberOfDays, startDate, possibleShift);
			...
			if (foundEntry != null && !foundEntry.getMapping().isEmpty()) {
				applyUserMapping(template, foundEntry.getMapping(), bookerName, numberOfPeople, numberOfBedrooms,
						maxDistanceFromLake, cityName, maxCityDistance, numberOfDays, startDate, possibleShift);
	...
	private RequestTemplate prepareRequestFromRdg(String rdgTurtle, String bookerName, String numberOfPeople,
			String numberOfBedrooms, String maxDistanceFromLake, String cityName, String maxCityDistance,
			String numberOfDays, String startDate, String possibleShift) {
	...
	private void applyUserMapping(RequestTemplate template, Map<String, AlignmentCandidate> mapping, String bookerName,
			String numberOfPeople, String numberOfBedrooms, String maxDistanceFromLake, String cityName,
			String maxCityDistance, String numberOfDays, String startDate, String possibleShift) {
	 ...
	 */
	private void setMappedLiteral(Resource subject, Model model, Map<String, AlignmentCandidate> mapping,
		...
		String remoteUri = candidate.getRemoteUri();
		if (remoteUri == null || remoteUri.trim().isEmpty())
			return;
	...
	private String buildJsonResponse(List<CottageResult> cottages, Boolean requiresMapping,
			Map<String, AlignmentCandidate> alignmentMap, String[] candidates) {
		System.out.println(requiresMapping);
		if (requiresMapping) {
			Gson gson = new Gson();

			MediatorResponse responseObj = new MediatorResponse(requiresMapping, cottages, alignmentMap, candidates);
	...
	private AlignmentResult buildAlignment(Model rdgModel, String cfNs, String alignmentId) {
		...
		String[] canonical = { "bookerName", "numberOfPeople", "numberOfBedrooms", "maxDistanceFromLake", "cityName",
				"nearestCity", "numberOfDays", "startDate", "possibleShift", "bookingNumber", "cottageID",
				"cottageName", "cottageAddress", "imageURL", "capacity", "distanceFromLake", "cityDistance",
...
//                System.out.println(score);
			...
			if (bestProp != null) {
				...
				latestAlignmentForUi.put(canon,
						new AlignmentCandidate(canon, bestRemoteName, bestProp.getURI(), bestScore));

				System.out.println("Alignment: " + canon + " -> " + bestProp.getURI() + " (score=" + bestScore + ")");

				if (bestScore < lowConfidenceThreshold) {
					System.out.println("  WARNING: low confidence for " + canon);
				}

				boolean perfect = canon.equals(bestRemoteName) && bestScore >= perfectThreshold;
				...
				latestAlignmentForUi.put(canon, new AlignmentCandidate(canon, null, null, -1));
				allMatchPerfectly = false;
			}
		}

		return new AlignmentResult(alignmentMap, new HashMap<>(latestAlignmentForUi), clientCandidates,
				allMatchPerfectly);
	}
	...
	public class MediatorResponse {
		...
		public MediatorResponse(boolean requiresMapping, List<CottageResult> cottages,
				Map<String, AlignmentCandidate> alignmentMap, String[] candidates) {
